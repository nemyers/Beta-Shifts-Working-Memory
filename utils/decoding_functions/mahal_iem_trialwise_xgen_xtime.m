function  [C2,cfg] = mahal_iem_trialwise_xgen_xtime(dat,theta,task,cfg)
% [distmat,cfg] = mahal_iem_trialwise_xgen_xtime(dat,theta,task,cfg)

if nargin < 3
    help mahal_iem_trialwise_xgen_xtime
...
%boolean vector of time indices to include in the decoding analysis
if ~isfield(cfg,'decodingtime') || isempty(cfg.decodingtime)
    cfg.decodingtime = true(1,ntime); end
decodingtime    = cfg.decodingtime;
ntraintime = nnz(decodingtime);
if islogical(decodingtime), traininds = find(decodingtime); else traininds = decodingtime; end
...
if returnConly
    ...
    C2      = nan(nChan,ntrl,ntraintime,ntraintime,2,'single');
            ...
            switch lower(distancefun)
                case {'mahal' 'mahalanobis' 'm' 'mah'}
                    ...
                    if covbytime
                        for ti = 1:ntraintime
                            for tj = 1:ntraintime
                                C2(:,testtrl,ti,tj,itask) = pdist2(dattest(:,:,traininds(tj)),D(:,:,traininds(ti)),'mahalanobis',covdiag(res(:,:,ti)))';
                            end
                        end                    
                    else
                        for ti = 1:ntraintime
                            for tj = 1:ntraintime
                                C2(:,testtrl,ti,tj,itask) = pdist2(dattest(:,:,traininds(tj)),D(:,:,traininds(ti)),'mahalanobis',sigma)';
                            end
                        end
                    end
                ...
                case {'euclid' 'euclidean' 'e' 'euc'}
                    for ti = 1:ntraintime
                        for tj = 1:ntraintime                  
                            C2(:,testtrl,ti,tj,itask) = pdist2(dattest(:,:,traininds(tj)),D(:,:,traininds(ti)),'euclidean')';
                        end
                    end
            end                        
        end
