function  [distmat,cfg] = mahal_iem_mds_xgen_1fold(dat,theta,task,cfg)
% [distmat,cfg] = mahal_iem_mds_xgen_1fold(dat,theta,task,cfg)
...
cfg.C = C;
%%
distmat = NaN(nChan,2,ntime,2,'single');
if verbose
   fprintf(' Decoding:'); r = ''; 
end
        ...
        case {'mahal' 'mahalanobis' 'm' 'mah'}
            ...
            if covbytime
                for ti = 1:ntime
                    ...
                    distmat(:,:,ti,itask) = mdscale(squareform(dists),2);                        
                end                    
            else
                for ti = 1:ntime
