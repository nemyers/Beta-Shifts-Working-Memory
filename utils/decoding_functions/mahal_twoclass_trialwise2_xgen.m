function  [C2,cfg] = mahal_twoclass_trialwise2_xgen(dat,labels,task,cfg)
% [C2,cfg] = mahal_twoclass_trialwise2_xgen(dat,labels,task,cfg)

if nargin < 4
    help mahal_twoclass_trialwise2_xgen
...
for ifold = 1:nfold
    ...
    for itask = 1:2
        ...
        currlabel = labels(traintrl);
        %get condition averages
        dtmp     = dat(traintrl,:,:);
        if demean, Dmu = mean(dtmp,1); dtmp = bsxfun(@minus,dtmp,Dmu); end
        D        = cat(1,mean(dtmp(currlabel==ulabels(1),:,:),1),mean(dtmp(currlabel==ulabels(2),:,:),1));
        ...
        switch lower(distancefun)
            case {'mahal' 'mahalanobis' 'm' 'mah'}
                ...
                res = bsxfun(@minus,dtmp(currlabel==ulabels(1),:,:),D(1,:,:));
                res = cat(1,res,bsxfun(@minus,dtmp(currlabel==ulabels(2),:,:),D(2,:,:)));    
