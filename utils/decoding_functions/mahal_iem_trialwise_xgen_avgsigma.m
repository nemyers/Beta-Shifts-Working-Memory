function  [C2,cfg] = mahal_iem_trialwise_xgen_avgsigma(dat,theta,task,cfg)
% [distmat,cfg] = mahal_iem_trialwise_xgen_avgsigma(dat,theta,task,cfg)

if nargin < 3
    help mahal_iem_trialwise_xgen_avgsigma
...
if returnConly
    ...
    for ifold = 1:nfold
        ...
        for itask = 1:2
            ...
            b        = pC*dtmp;
            D        = [D; reshape(b,[nChan+constant nsens ntime])];
            if add_constant, Dmu = D(end,:,:); D(end,:,:) = []; end
            res = [res; reshape(dtmp - [b'*C(:,traintrl)]',[nnz(traintrl) nsens ntime])];
        end
        clear pC dtmp b
        ...
        if covbytime
            for ti = 1:ntime
                tmpdist = pdist2(dattest(:,:,ti),D(:,:,ti),'mahalanobis',covdiag(res(:,:,ti)))';
                C2(:,testtrl,ti,1) = tmpdist(1:nChan,:);
                C2(:,testtrl,ti,2) = tmpdist((nChan+1):end,:);
            end                    
        else
            for ti = 1:ntime
                tmpdist = pdist2(dattest(:,:,ti),D(:,:,ti),'mahalanobis',sigma)';
                C2(:,testtrl,ti,1) = tmpdist(1:nChan,:);
                C2(:,testtrl,ti,2) = tmpdist((nChan+1):end,:);                
            end
        end
