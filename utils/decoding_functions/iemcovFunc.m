function [C,Cproj] = iemcovFunc(dat,theta,thetabasis,foldvar,covtime)
% [C,Cproj] = iemcovFunc(dat,theta,thetabasis,foldvar,covtime)    
...
if get_projection
    if recenter_curve
        costheta = repmat(cos(thetabasis - angspace(ceil(nChan/2)+1)),[1 ntrl ntime]); %get weight for each virtual channel
        Cproj    = squeeze(sum(costheta.*C,1)); %get cosine-weighted average of virtual channel responses
    else
        costheta = repmat(cos(repmat(thetabasis,[1 ntrl])-repmat(theta',[nChan 1])),[1 1 ntime]); %get weight for each virtual channel
        Cproj    = squeeze(sum(costheta.*C,1)); %get cosine-weighted average of virtual channel responses
    end
else
    Cproj = [];
