function  [C2,cfg] = mahal_lin_trialwise_xtime(dat,theta,cfg)
% [C2,cfg] = mahal_lin_trialwise_xtime(dat,theta,cfg)

% authors: M Stokes, E Spaak, N Myers

if nargin < 2
    help mahal_lin_trialwise_xtime
...
if ~isfield(cfg,'thetabasis') || isempty(cfg.thetabasis)
    cfg.thetabasis = unique(round(theta*1000)/1000);
...
C = double(theta'==thetabasis);
if add_constant
    C(nChan+1,:) = 1;%add constant regressor to model
end
cfg.C = C;
%%
C2      = nan(nChan,ntrl,ntime,ntime,'single');
    ...
    switch lower(distancefun)
        case {'mahal' 'mahalanobis' 'm' 'mah'}
            ...
            if ~covbytime, nfeat = nnz(traintrl)*nnz(covtime);
                    sigma = covdiag(reshape(permute(res(:,:,covtime),[1 3 2]),nfeat,nsens)); end
            if covbytime
                for ti = 1:ntime
                    sigma = covdiag(res(:,:,ti));
                    curdat = D(:,:,ti);
                    for tj = 1:ntime
                        C2(:,testtrl,ti,tj) = pdist2(dattest(:,:,tj),curdat,'mahalanobis',sigma)';
                    end
                end                    
            else
                for ti = 1:ntime
        ...
        case {'euclid' 'euclidean' 'e' 'euc'}
            for ti = 1:ntime
                curdat = D(:,:,ti);
                for tj = 1:ntime
                    C2(:,testtrl,ti,tj) = pdist2(dattest(:,:,ti),curdat,'euclidean')';
                end
...
if recenter_curve
    C        = zeros(nChan,ntrl,ntime,ntime,'single');
    chanlist = 1:nChan; 
    for itrl = 1:ntrl
        iang     = testangles(itrl);
        idst     = thetabasis-iang;
        [~,imin] = min(abs(idst));
