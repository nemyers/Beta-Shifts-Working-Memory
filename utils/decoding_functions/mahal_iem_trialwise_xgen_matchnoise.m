...
if returnConly
    ...
    for ifold = 1:nfold
        ...
        %match snr across training sets
        dtmp_task = {};
        D = {};
        sigma_task = {};
        for itask =1:2
            ...
            dtmp_task{itask} = dat(traintrl,:,:);
            dtmp_task{itask} = reshape(dtmp_task{itask},[nnz(traintrl) nsens*ntime]);
            b        = pC*dtmp_task{itask};
            D{itask} = reshape(b,[nChan+constant nsens ntime]);
            
            % get covariance estimate
            if (~covbytime | itask==1 | (covbytime & ~covwithintask)), res      = reshape(dtmp_task{itask} - [b'*C(:,traintrl)]',[nnz(traintrl) nsens ntime]); end;
            if (~covbytime & (itask==1 | ~covwithintask)), sigma_task{itask} = covdiag(mean(res(:,:,covtime),3)); end %covdiag or cov                    
            distmat = pdist(D{itask},'mahalanobis',sigma_task{itask})';
            if itask==2,distmat2 = distmat;end;
            E(:,itask) = eig(squareform(distmat));
            %distvar(:,itask) = sum(E(1:2,itask));
            distvar(:,itask) = sum(abs(E(1:2,itask)))./sum(abs(E(3:end-1,itask)));
        end
        
        %signalratio = distvar(1)./distvar(2);
        %randomise sigma
        sigdiag = diag(sigma_task{1});
        sigoffdiag = squareform(sigma_task{1}-diag(sigdiag));
        sigoffdiag = squareform(sigoffdiag(randperm(length(sigoffdiag))));
        signoise = sigoffdiag + diag(sigdiag(randperm(length(sigdiag))));
        randvec = randperm(size(sigma_task{1},1));
        signoise = sigma_task{1}(randvec,randvec);
        %added_noise = mvnrnd(zeros(nsens,1),sigma_task{1},size(dtmp_task{1},1));
        added_noise = mvnrnd(zeros(nsens,1),signoise,size(dtmp_task{1},1));
        
        %noise = mvnrnd(zeros(nchans,1),sigma1,ntimes);
        noisescaling = 0.05;
        nrnd = 0;
        
        distvars = [];
        while abs(distvar(1)./distvar(2))>1
           nrnd = nrnd+1;
           for itask = 1
                if itask == 1, traintrl = traintrl1; else traintrl = traintrl2; end
                ...
                dtmp     = dtmp_task{itask} + added_noise*noisescaling*nrnd*(2-itask);
                ...
                distmat = pdist(D,'mahalanobis',sigma)';
                E(:,itask) = eig(squareform(distmat));
                Es(:,nrnd) = E(:,itask);
                %distvar(:,itask) = sum(E(1:2,itask));
                %distvars(1,nrnd) =  sum(E(1:2,itask));
                distvar(:,itask) = sum(abs(E(1:2,itask)))./sum(abs(E(3:end-1,itask)));
                distvars(1,nrnd) = sum(abs(E(1:2,itask)))./sum(abs(E(3:end-1,itask)));
            end
        end
        ...
        for itask = 1:2
            ...
            pC       = pinv(C(:,traintrl)');
            %dtmp     = dat(traintrl,:,:);
            dtmp     = dtmp_task{itask} + added_noise*noisescaling*nrnd*0*(2-itask);
            
            %[~,table] = anovan(dat(traintrl,:,:),{C(:,traintrl)'});
