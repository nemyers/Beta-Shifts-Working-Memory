function  [C2,cfg] = mahal_twoclass_trialwise(dat,labels,cfg)
% [C2,cfg] = mahal_twoclass_trialwise(dat,labels,cfg)
...
X = [labels ones(length(labels),add_constant)];
X(:,1) = X(:,1) - min(X(:,1));
X(:,1) = X(:,1)./max(X(:,1));
nreg = size(X,2);
%%
    ...
    case {'mahal' 'mahalanobis' 'm' 'mah'}
        for ifold = 1:nfold
            ...
            if add_constant
                ...
                D(end,:,:) = [];
                for ti = 1:ntime
                    C2(:,testtrl,ti) = pdist2(bsxfun(@minus,dat(testtrl,:,ti),Dmu(1,:,ti)),D(:,:,ti),'mahalanobis',sigma)';
                end
            else                
                for ti = 1:ntime                   
                    C2(:,testtrl,ti) = pdist2(dat(testtrl,:,ti),D(:,:,ti),'mahalanobis',sigma)';
                end                
            end            
        end
    case {'euclid' 'euclidean' 'e' 'euc'}
        for ifold = 1:nfold
            ...
            if add_constant
                ...
                D(end,:,:) = [];
                for ti = 1:ntime
                    C2(:,testtrl,ti) = pdist2(bsxfun(@minus,dat(testtrl,:,ti),Dmu(1,:,ti)),D(:,:,ti),'euclidean')';
                end
            else                
                for ti = 1:ntime                   
                    C2(:,testtrl,ti) = pdist2(dat(testtrl,:,ti),D(:,:,ti),'euclidean')';
                end                
            end
        end
end
