function  [C2,cfg] = mahal_iem_trialwise_xtime(dat,theta,cfg)
% [C2,cfg] = mahal_iem_trialwise_xtime(dat,theta,cfg)

% authors: M Stokes, E Spaak, N Myers

if nargin < 2
    help mahal_iem_trialwise_xtime
    ...
    switch lower(distancefun)
        case {'mahal' 'mahalanobis' 'm' 'mah'}
            ...
            if covbytime
                for ti = 1:ntime
                    for tj = 1:ntime
                        C2(:,testtrl,ti,tj) = pdist2(dattest(:,:,tj),D(:,:,ti),'mahalanobis',covdiag(res(:,:,ti)))';
                    end
                end                    
            else
                for ti = 1:ntime
                    for tj = 1:ntime
                        C2(:,testtrl,ti,tj) = pdist2(dattest(:,:,tj),D(:,:,ti),'mahalanobis',sigma)';
                    end
                end
            end
        ...
        case {'euclid' 'euclidean' 'e' 'euc'}
            for ti = 1:ntime    
                for tj = 1:ntime
                    C2(:,testtrl,ti,tj) = pdist2(dattest(:,:,tj),D(:,:,ti),'euclidean')';
                end
            end
    end
end
