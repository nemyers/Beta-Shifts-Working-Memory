function  [mante_proj,variance_proj,cfg] = mante_proj_xgen(dat,theta,task,cfg)
% [mante_proj,variance_proj,cfg] = mante_proj_xgen(dat,theta,task,cfg)
    ...
    for ifold = 1:nfold
        ...
        for itask = 1:3
            ...
            for it = 1:ntime
                ...
                curdat     = curproj(1:nreg,NotNaN,it,itask,ifold)';
                curmu      = mean(curdat,1);
                reducedvar = sum(pdist2(curdat,curmu).^2,1)./(nChan-1);
                
                cur_variance_proj(:,it,itask,ifold,2) = [fullvar reducedvar];
                
                fullvar    = mean(pdist(muerf(:,NotNaN,it)'));
                reducedvar = mean(pdist(curproj(1:nreg,NotNaN,it,itask,ifold)'));
                cur_variance_proj(:,it,itask,ifold,3) = [fullvar reducedvar];
            end
        end
