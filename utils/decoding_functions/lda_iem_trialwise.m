function  [C2,cfg] = lda_iem_trialwise(dat,theta,cfg)
% [C2,cfg] = lda_iem_trialwise(dat,theta,cfg)
% author: N Myers

if nargin < 2
    help lda_iem_trialwise
...
cfg.C = C;
%%
C2      = nan(nChan,ntrl,ntime,'single');
...
for ifold = 1:nfold
    ...
    if add_constant || demean
        ...
        if ~covbytime, sigma = covdiag(mean(res(:,:,covtime),3)); psigma = pinv(sigma); end %covdiag or cov                
        if covbytime
            for ti = 1:ntime
                sigma = covdiag(res(:,:,ti));
                Sb    = covdiag(D(:,:,ti));
                [V DD] = eig(pinv(sigma)*Sb);
                eigvals = diag(DD);
                [eigsort,eigsortind] = sort(eigvals,'descend');
                V = V(:,eigsortind);
                proj = dattest(:,:,ti)*V;
                C2(:,testtrl,ti,itask) = proj;
            end                    
        else
            for ti = 1:ntime
                Sb    = covdiag(D(:,:,ti));
