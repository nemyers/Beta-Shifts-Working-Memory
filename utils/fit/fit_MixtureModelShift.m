function [fit,cfg] = fit_MixtureModelShift(x,data,bias,cfg)
%        [fit,cfg] = fit_MixtureModelShift(x,data,bias,cfg)
%
% fit mixture model by minimising MSE
    ...
    function ll = mle_mixpdfVM(x, y, bias, param)
        ...
        K2 = param(2);
        Pm = param(3);
        if length(param)<4, mu = 0; else, mu = param(4); end
        p  = circ_vmpdf(x,mu-bias*K2,K)*Pm + size(x,2)/(2*pi)*(1-Pm);
        %p  = reshape(p,size(x));
        p  = p./abs(sum(p,2));
        %ll = -sum(log(p));
